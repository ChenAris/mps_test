...
#    batch_sizes = [196,128,96,64,48,32,16,8,4,2,1]
    test_times = 5
	...
	if batch_sizes < 32:
        ...
        for j in range(test_times):
            gpu_uti,throughput,latency,cpu_uti,memory_gpu = run_inference(batch_size,model_name,loop_number,3)
	...
	avg_memory_gpu = avg_memory_gpu / test_times
        fname = open(model_name+'_async.log','a+')
...
def main():
#    model_name = ['squee','resnet']
    model_name = ['vgg'] 
    for model in model_name:
	os.popen('rm '+model+'_async.log')
	run_main(model)
